...
class Admin:
    ...
    def admin_main_menu(self):
        ...
        self.fineimg = Button(self.mainframe, image=self.mi7, border=0, bg=NAV_COLOR, command=self.fine)
        self.fineimg.grid(column=0, row=11, pady=(10, 0))
        self.label7 = Label(self.mainframe, text="Fine", bg=NAV_COLOR, fg=NAV_TEXT_COLOR)
        self.label7.grid(column=0, row=12, pady=(5, 20))
    ...
    def mainsearch(self, f):
        ...
        if f == 1:
            ...
            self.searchframe.place(x=350, y=185)
            self.searchframe.config(width=520)
            self.searchbut.config(command=self.searchfine)
            self.searchbut.config(text="Search Username")
            self.searchbut.place(x=0, y=23)
            self.searchentry.config(width=18, textvariable=self.searchvar)
            self.searchentry.place(x=195, y=25, height=35)
            self.resetbut.config(command=self.resetfine)
            self.resetbut.place(x=415, y=23)
            self.cur.execute("select user from sales")
            li = self.cur.fetchall()
            a = []
            for i in range(0, len(li)):
                a.append(li[i][0])
    ...
    def searchfine(self):
        if self.searchvar.get() == '':
            return
        self.tree.delete(*self.tree.get_children())
        self.cur.execute("select Date, due_date, user, fine from sales")
        li = self.cur.fetchall()
        for i in li:
    ...
    def additems(self):
        ...
        for i in range(0, len(list_)):
            Label(self.itemframe, text=list_[i], font="Roboto 14 bold", bg="#ffffff").grid(row=i, column=0, pady=15,sticky="w")
    ...
    def insertitem(self):
        ...
        self.cur.execute("insert into products values(?,?,?,?,?,?)", (x, self.newitem.get(), self.newitemdesc.get(),self.newitemcat.get(), y, z))
        self.newitem.set('')
    ...
    def buildsalestable(self):
        ...
        self.itemframe.place_forget()
        self.formframe1.place_forget()
        self.tableframe1.place(x=1475, y=255, anchor=NE)
        self.tree.delete(*self.tree.get_children())
        self.tree.grid_remove()
        self.tree.destroy()
        ...
        self.tree = ttk.Treeview(self.tableframe1,
                                 columns=("Transaction ID", "Invoice No.", "Product ID", "Product",
                                          'Quantity', 'Total Price', 'Date', 'Time', 'Due Date','User','Fine'),
    ...
    def getsales(self,n):
        if n == 1:
            ...
            for i in range(0, len(saleslist)):
                ...
                self.cur.execute("select product_name,product_price from products where product_id=?",(int(saleslist[i][2]),))
                list_ = self.cur.fetchall()
                ...
                saleslist[i] = [saleslist[i][0], saleslist[i][1], saleslist[i][2], list_[0][0], saleslist[i][3],
                                list_[0][1] * (int(saleslist[i][3])), saleslist[i][4], saleslist[i][5], saleslist[i][6], saleslist[i][7], saleslist[i][8]]
            ...
            for i in range(0, len(saleslist)):
                ...
                s = (str(saleslist[i][4])).split('-')
                saleslist[i][4] = s[2] + " - " + s[1] + " - " + s[0]
                # self.cur.execute("INSERT INTO fine VALUES(?,?,?,?)", (saleslist[i][4],saleslist[i][6],saleslist[i][7],saleslist[i][8]))
                saleslist[i] = [saleslist[i][4],saleslist[i][6],saleslist[i][7],saleslist[i][8]]
                saleslist[i] = tuple(saleslist[i])
            for i in saleslist:
                self.tree.insert('', 'end', values=i)
    ...
    def searchinvoice(self):
        ...
        for i in range(0, len(saleslist)):
            ...
            saleslist[i] = [saleslist[i][0], saleslist[i][1], saleslist[i][2], list_[0][0], saleslist[i][3],
                            ...
                            saleslist[i][4], saleslist[i][5], saleslist[i][6], saleslist[i][7],saleslist[i][8]]
            saleslist[i] = tuple(saleslist[i])
        for j in saleslist:
    ...
    def fine(self):
        ...
        for sale in sales:
            if sale[6] is not None:
                sale_date = datetime.strptime(sale[6], "%Y-%m-%d").date()
                # print(sale_date)
        ...
        self.itemframe.place_forget()
        self.formframe1.place_forget()
        self.tableframe1.place(x=1000, y=255, anchor=NE)
        self.tree.delete(*self.tree.get_children())
        self.tree.grid_remove()
        self.tree.destroy()
        self.active(self.label2)
        ...
        scrollbary = Scrollbar(self.tableframe1, orient=VERTICAL)
        self.tree = ttk.Treeview(self.tableframe1,
                                 columns=('Date', 'Due Date','User','Fine'),
                                 selectmode="browse", height=16,
                                 yscrollcommand=scrollbary.set, xscrollcommand=scrollbarx.set)
        self.tree.column('#0', stretch=NO, minwidth=0, width=0)
        self.tree.column('#1', stretch=NO, minwidth=0, width=140)
